...
async function getLeetCodeData() {
  const LEETCODE_USERNAME = "prathamesh_1828";
...
export default async function LeetcodeActivity() {
  const calendarData = await getLeetCodeData();
  ...
  if (calendarData && calendarData.submissionCalendar) {
    ...
    // Parse UNIX timestamps to YYYY-MM-DD
    ...
    startDate.setDate(today.getDate() - 364 + 1);
    ...
    weeks.forEach((week, i) => {
      if (week.contributionDays.length > 0) {
        const dateParts = week.contributionDays[0].date.split('-');
        ...
        <GlassCard className="p-5 md:p-8 overflow-x-auto overflow-y-hidden touch-pan-x relative min-h-[200px] w-full max-w-full">
          {!calendarData ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
               ...
               <span className="text-sm text-zinc-500">Unable to load LeetCode activity.</span>
            </div>
          ) : (
            <div className="min-w-[700px] flex flex-col gap-4">
              <div className="flex justify-between items-end text-sm text-zinc-400">
                <div>
                  <span className="font-semibold text-zinc-200">{totalSubmissions.toLocaleString()}</span> submissions in the past one year
                </div>
                <div className="flex gap-4">
                  <span>Total active days: <span className="font-semibold text-zinc-200">{calendarData.totalActiveDays}</span></span>
                  <span>Max streak: <span className="font-semibold text-zinc-200">{calendarData.streak}</span></span>
                </div>
              </div>
                    ...
                    {week.contributionDays.map((day: ContributionDay, j: number) => (
                      ...
                      >
                        ...
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block px-2.5 py-1.5 bg-[#222] text-xs text-zinc-300 rounded-md whitespace-nowrap z-50 shadow-[0_10px_40px_rgba(0,0,0,0.5)] border border-white/10 pointer-events-none">
                          <span className="font-semibold text-white">{day.contributionCount > 0 ? day.contributionCount : "No"}</span> submissions on {new Date(day.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                        </div>
                      </div>
                    ))}
                  </div>
